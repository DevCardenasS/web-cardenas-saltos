...
export default function Navbar({ transparent }) {
  ...
  // Si la página pide transparencia Y no hemos bajado scroll Y el móvil está cerrado -> Transparente
  const isTransparentNow = transparent && !isScrolled && !isOpen;

  return (
    <nav 
      className={`w-full z-[100] font-['Gantari'] font-normal transition-all duration-500 border-b border-white/10 ${
        isTransparentNow 
          ? 'bg-transparent fixed top-0 left-0 border-transparent' 
          : 'bg-[#051d40] sticky top-0 border-white/20'
      }`}
    >
